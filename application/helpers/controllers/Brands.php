...
class Brands extends MY_Controller {
	...
	public function company_add(){
		$this->permission_check('brand_add');
		$data = $this->data;
		$data['page_title']=$this->lang->line('brand_company');
		$this->load->view('company_add', $data);
	}
	public function company_view(){
		$this->permission_check('brand_view');
		$data=$this->data;
		$data['page_title']=$this->lang->line('brand_company_list');
		$this->load->view('company_view', $data);
	}

	public function new_brand_company(){
		$this->form_validation->set_rules('brand', 'Brand Company', 'trim|required');
			$brand = $this->input->post('brand');
			$description = $this->input->post('description');
		if ($this->form_validation->run() == TRUE) {
			$qs4="select coalesce(max(id),0)+1 as maxid from db_brand_companies";
			$q1=$this->db->query($qs4);
			$maxid=$q1->row()->maxid;
			$cat_code='CT'.str_pad($maxid, 4, '0', STR_PAD_LEFT);

			$query1="insert into db_brand_companies(company_code,company_name,description,status)
	...
	public function update_brand_company(){
		$this->permission_check_with_msg('brand_edit');
		$id=$this->input->post('id');
		$status=$this->input->post('status');
	...
	{
		$list = $this->brand->get_datatables2();
