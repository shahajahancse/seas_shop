	...
	class Users extends MY_Controller{
		...
		public function save_or_update(){
			...
			if($this->input->post('command')!='update'){
				$this->form_validation->set_rules('newpass', 'Password', 'required|trim|min_length[5]|max_length[12]');
			}
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if($this->input->post('command')!='update'){
					$password=md5($this->input->post('newpass'));//Encrypted in MD5
		...
		public function password_update(){
			/*if($this->session->userdata('inv_username')=='admin'){
